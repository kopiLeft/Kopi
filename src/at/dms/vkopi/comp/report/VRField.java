 ...
 *
 * $Id: VRField.java,v 1.1 2004/07/28 18:43:29 imad Exp $
 */
...
public class VRField
  extends VKPhylum
  implements VRConstants, at.dms.kopi.comp.kjc.Constants
{
  ...
  public boolean isHidden() {
    return (options & at.dms.vkopi.lib.report.Constants.CLO_HIDDEN) > 0;
  }
  ...
  public void checkCode(VKContext context, VRReport report) throws PositionedError {
    ...
    if (type.getDef() instanceof VKFixedType) {
      align = at.dms.vkopi.lib.report.Constants.ALG_RIGHT;
    }
    if (ident == null) {
  ...
  public boolean wantReturn(int event) {
    switch (at.dms.vkopi.lib.report.Constants.TRG_TYPES[event]) {
    case at.dms.vkopi.lib.report.Constants.TRG_VOID:
      return false;
    case at.dms.vkopi.lib.report.Constants.TRG_OBJECT:
      return true;
    default:
...
*/
  ...
  public JExpression genConstructorCall() {
    ...
    if (type.getDef() instanceof VKCodeType) {
      params.addElement(((VKCodeType)type.getDef()).genNames());
    ...
    return new JAssignmentExpression(ref,
				     ...
				     new JUnqualifiedInstanceCreation(ref,
							      type.getDef().getReportType(),
							      (JExpression[])at.dms.util.base.Utils.toArray(params, JExpression.class)));
  }
  ...
  private JExpression getCompute() {
    ...
    for (int i = 0; i < triggers.length; i++) {
      if ((triggers[i].getEvents() & (1 << at.dms.vkopi.lib.report.Constants.TRG_COMPUTE)) > 0) {
	JExpression	expr;
	...
	expr = new JMethodCallExpression(ref,
					 ...
					 "callTrigger",
					 new JExpression[] {
					   VKUtils.toExpression(ref, at.dms.vkopi.lib.report.Constants.TRG_COMPUTE),
  ...
  private JExpression getFormat() {
    ...
    for (int i = 0; i < triggers.length; i++) {
      if ((triggers[i].getEvents() & (1 << at.dms.vkopi.lib.report.Constants.TRG_FORMAT)) > 0) {
	JExpression	expr;
	...
	expr = new JMethodCallExpression(ref,
					 ...
					 "callTrigger",
					 new JExpression[]{
					   VKUtils.toExpression(ref, at.dms.vkopi.lib.report.Constants.TRG_FORMAT),
					   VKUtils.toExpression(ref, getIndex() + 1)
					 });
  ...
  private static final CType	COMPUTE_TYPE = CReferenceType.lookup(at.dms.vkopi.lib.report.VCalculateColumn.class.getName().replace('.','/'));
  private static final CType	FORMAT_TYPE = CReferenceType.lookup(at.dms.vkopi.lib.report.VCellFormat.class.getName().replace('.','/'));
