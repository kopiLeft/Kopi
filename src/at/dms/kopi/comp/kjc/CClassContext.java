 ...
 *
 * $Id: CClassContext.java,v 1.2 2004/09/29 16:22:07 taoufik Exp $
 */
...
public class CClassContext extends CContext {
  ...
  CClassContext(CContext parent, KjcEnvironment environment, CSourceClass clazz, JTypeDeclaration decl) {
   ...
   /**
   *  @return The enum inner class context if it exist, null otherwise 
   */
  public  JEnumSwitchInnerDeclaration getEnumInnerClass(String ident) throws PositionedError {
    JEnumSwitchInnerDeclaration  innerEnumMap;
    
    innerEnumMap = (JEnumSwitchInnerDeclaration) enumMap.get(ident);
    if (innerEnumMap == null) {     
       innerEnumMap = new JEnumSwitchInnerDeclaration(decl.getTokenReference(), ident, this);
       innerEnumMap.init(this);
       enumMap.put(ident, innerEnumMap);
    }
  ...
  {
    //enumMap
    for (Iterator it = enumMap.values().iterator(); it.hasNext(); ) {
      ((JEnumSwitchInnerDeclaration)it.next()).close(this);
    }
   ...
   *
   * @return true if the context is static
   */
