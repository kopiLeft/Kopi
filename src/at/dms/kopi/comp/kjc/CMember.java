 ...
 *
 * $Id: CMember.java,v 1.1 2004/07/28 18:43:28 imad Exp $
 */
 ...
 * @see	CClass
 */
public abstract class CMember extends at.dms.util.base.Utils implements Constants {
  ...
  public boolean requiresAccessor(CClass from, boolean isSuper) {
    ...
    if (getOwner() == from) {
      return false;
    } else if (!isPrivate() && from.descendsFrom(getOwner())) {
      return false;
    } else if (!from.descendsFrom(getOwner()) && isSuper) {
      return true;
    } else if (isProtected() && !from.descendsFrom(getOwner()) && !from.getPackage().equals(getOwner().getPackage())) {
      return true;
    } else if (!isPrivate())  {
      return false;
    } else {
