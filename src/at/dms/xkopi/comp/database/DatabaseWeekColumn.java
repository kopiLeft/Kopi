 ...
 *
 * $Id: DatabaseWeekColumn.java,v 1.1 2004/07/28 18:43:29 imad Exp $
 */
...
/**
 * The type of a field which represents a week-column in Database.k. 
 */
public class DatabaseWeekColumn extends DatabaseColumn{

  /**
   * Creates a representation of a column with type Week. The values in 
   * the column are NOT restricted.
   */
  public DatabaseWeekColumn(boolean isNullable) {
  ...
  protected CType getStandardType(boolean isNullable) {
    return isNullable ? XStdType.Week : XStdType.PWeek;
  } 
  ...
  public JExpression getCreationExpression() {
    return new JUnqualifiedInstanceCreation(TokenReference.NO_REF,
                            new CClassNameType(TokenReference.NO_REF, DatabaseWeekColumn.class.getName().replace('.','/')),
                            new JExpression[]{
  ...
  public boolean isEquivalentTo(DatabaseColumn other, int check) {
    if (other instanceof DatabaseWeekColumn) {
