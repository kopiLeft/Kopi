 ...
 *
 * $Id: DatabaseTimeColumn.java,v 1.1 2004/07/28 18:43:29 imad Exp $
 */
 ...
 */
public class DatabaseTimeColumn extends DatabaseColumn{

  /**
   * Creates a representation of a column with type Date. The values in 
   * the column are NOT restricted.
   */
  public DatabaseTimeColumn(boolean isNullable) {
   ...
   * @return the type
   */
  public CType getStandardType(boolean isNullable) {
    return isNullable ? XStdType.Time : XStdType.PTime;
  } 
  ...
  public JExpression getCreationExpression() {
    return new JUnqualifiedInstanceCreation(TokenReference.NO_REF,
                            new CClassNameType(TokenReference.NO_REF, DatabaseTimeColumn.class.getName().replace('.','/')),
                            new JExpression[]{
  ...
  public boolean isEquivalentTo(DatabaseColumn other, int check) {
    if (other instanceof DatabaseTimeColumn) {
