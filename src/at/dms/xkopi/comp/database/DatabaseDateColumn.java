 ...
 *
 * $Id: DatabaseDateColumn.java,v 1.1 2004/07/28 18:43:29 imad Exp $
 */
 ...
 */
public class DatabaseDateColumn extends DatabaseColumn{

  /**
   * Creates a representation of a column with type date. The values in 
   * the column are NOT restricted.
   */
  public DatabaseDateColumn(boolean isNullable) {
  ...
  protected CType getStandardType(boolean isNullable) {
    return  isNullable ?  XStdType.Date : XStdType.PDate;
  } 
  ...
  public JExpression getCreationExpression() {
    return new JUnqualifiedInstanceCreation(
                            ...
                            new CClassNameType(TokenReference.NO_REF, 
                              DatabaseDateColumn.class.getName().replace('.','/')),
                            new JExpression[]{
  ...
  public boolean isEquivalentTo(DatabaseColumn other, int check) {
    if (other instanceof DatabaseDateColumn) {
