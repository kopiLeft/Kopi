...
public class VRField
  extends VKPhylum
  implements VRConstants, com.kopiright.kopi.comp.kjc.Constants
{
  ...
  public boolean wantReturn(int event) {
    switch (com.kopiright.vkopi.lib.report.Constants.TRG_TYPES[event]) {
    case com.kopiright.vkopi.lib.report.Constants.TRG_VOID:
      return false;
    case com.kopiright.vkopi.lib.report.Constants.TRG_OBJECT:
      return true;
    default:
...
*/
  ...
  private JExpression getCompute() {
    ...
    for (int i = 0; i < triggers.length; i++) {
      if ((triggers[i].getEvents() & (1 << com.kopiright.vkopi.lib.report.Constants.TRG_COMPUTE)) > 0) {
	JExpression	expr;
  ...
  private JExpression getFormat() {
    ...
    for (int i = 0; i < triggers.length; i++) {
      if ((triggers[i].getEvents() & (1 << com.kopiright.vkopi.lib.report.Constants.TRG_FORMAT)) > 0) {
	JExpression	expr;
	...
	expr = new JMethodCallExpression(ref,
					 ...
					 "callTrigger",
					 new JExpression[]{
					   VKUtils.toExpression(ref, com.kopiright.vkopi.lib.report.Constants.TRG_FORMAT),
					   VKUtils.toExpression(ref, getIndex() + 1)
					 });
  ...
  private static final CType	COMPUTE_TYPE = CReferenceType.lookup(com.kopiright.vkopi.lib.report.VCalculateColumn.class.getName().replace('.','/'));
  private static final CType	FORMAT_TYPE = CReferenceType.lookup(com.kopiright.vkopi.lib.report.VCellFormat.class.getName().replace('.','/'));
