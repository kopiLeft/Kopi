...
public class PRHorizontalBlock extends PRBlock {
  ...
  public JExpression genConstructorCall() {
    ...
    for (int i = 0; i < blocks.length; i++) {
      init[i] = VKUtils.toExpression(ref, blocks[i].getIdent());
    }
  ...
  private static CReferenceType	TYPE	= CReferenceType.lookup(com.kopiright.vkopi.lib.print.PHorizontalBlock.class.getName().replace('.','/'));
