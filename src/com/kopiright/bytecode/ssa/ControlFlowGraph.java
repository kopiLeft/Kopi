...
import com.kopiright.bytecode.classfile.AccessorContainer;
import com.kopiright.bytecode.classfile.AccessorTransformer;
...
import com.kopiright.bytecode.classfile.HandlerInfo;
import com.kopiright.bytecode.classfile.Instruction;
import com.kopiright.bytecode.classfile.InstructionAccessor;
...
public class ControlFlowGraph {
    ...
    for (BasicBlock bb = firstBB; bb != null; bb = bb.getNext()){
      if (!accessibleBlocks.contains(bb)) {
        ...
        for (int i = 0; i < exceptionHandlers.length; ++i) {
          exceptionHandlers[i].removeProtectedBlock(bb);
        }
        // if the last instruction is a subroutine call, remove this call.
        Edge defaultNext = bb.getDefaultNext();
        if (defaultNext != null) {
          SubRoutine sub = (SubRoutine) subroutines.get(defaultNext.getTarget());
          if (sub != null) {
            sub.removeCall(defaultNext);
          }
        }
        graph.removeNode(bb);
      }
  ...
  public HandlerInfo[] getHandlerInfos(Instruction[] insts) {
    ...
    handlers = new HandlerInfo[newHandlers.size()];
    newHandlers.toArray(handlers);
    return handlers;
  }
  ...
  private void findCFGEdges(Instruction[] insts, short[] numBlocks,
    ...
    for(int i = 0; i < bbs.length; ++i) {
      ...
      if (lastInst instanceof JumpInstruction) {
        ...
        if (lastInst.getOpcode() == Constants.opc_goto || lastInst.getOpcode() == Constants.opc_goto_w) {
        ...
        } else if (lastInst.getOpcode() == Constants.opc_jsr || lastInst.getOpcode() == Constants.opc_jsr_w) {
          //do nothing here
  ...
  public void findSubRoutines(short[] numBlocks,
    ...
    for(int i = 0; i < bbs.length; ++i) {
      ...
      if (lastInst.getOpcode() == Constants.opc_jsr
          || lastInst.getOpcode() == Constants.opc_jsr_w)
      {
        JumpInstruction jumpInst = (JumpInstruction) lastInst;
        ...
        if (sub == null) {
          ...
          specialVisitGraphFromNode(graph, insts, currentBB, new NodeVisitor() {
              public boolean visit(Node n) {
          ...
          if ((sub = (SubRoutine)subroutines.get(startSubroutine)) == null) {
            // there is no ret instruction for this subroutine (this is possible !)
            sub = new SubRoutine(startSubroutine, null);
            subs.put(startSubroutine, sub);
          }
    ...
    for(int i = 0; i < bbs.length; ++i) {
      Instruction lastInst = insts[bbs[i].getEnd()];
      if (lastInst.getOpcode() == Constants.opc_jsr || lastInst.getOpcode() == Constants.opc_jsr_w) {
        BasicBlock subroutineCall = bbs[i];
        BasicBlock subroutineReturn = bbs[i+1];
   ...
   * This search doesn't follow the edges associated with a subroutine call.
   *
   * @param node the search source
   * @param nodeVisitor the node visitor
   */
  public void specialVisitGraphFromNode(Graph g,
                                        final Instruction[] insts,
                                        Node start,
                                        NodeVisitor nodeVisitor)
  {
    ...
    specialVisitNode(start, nodeVisitor, insts);
  }

  /**
   * Visit a graph in depth first search from a given node
   * This visit doesn't follow the edges associated with a subroutine call.
   *
   * @param node the search source
   * @param nodeVisitor the node visitor
   */
