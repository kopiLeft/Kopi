...
header { package com.kopiright.bytecode.ksm; }
...
:
  token : INT_LITERAL
    {
      try {
	self = new Integer(com.kopiright.compiler.base.NumberParser.decodeInt(token.getText()));
      } catch (NumberFormatException e) {
	throw new RecognitionException(e.toString());
      }
    }
...
:
  token : LONG_LITERAL
    {
      try {
	self = new Long(com.kopiright.compiler.base.NumberParser.decodeLong(token.getText().substring(0, token.getText().length() - 1)));
      } catch (NumberFormatException e) {
	throw new RecognitionException(e.toString());
      }
    }
