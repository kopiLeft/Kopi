...
header { package org.kopi.bytecode.ksm; }
...
:
  token : INT_LITERAL
    {
      try {
	self = new Integer(org.kopi.compiler.base.NumberParser.decodeInt(token.getText()));
      } catch (NumberFormatException e) {
	throw new RecognitionException(e.toString());
      }
    }
...
:
  token : LONG_LITERAL
    {
      try {
	self = new Long(org.kopi.compiler.base.NumberParser.decodeLong(token.getText().substring(0, token.getText().length() - 1)));
      } catch (NumberFormatException e) {
	throw new RecognitionException(e.toString());
      }
    }
