...
public class JClassDeclaration extends JTypeDeclaration {
  ...
  {
    super(where, modifiers, ident, typeVariables, interfaces, fields, 
          methods, inners, initializers, javadoc, comment);
  ...
  public void join(CContext context) throws PositionedError {
	...
	// superClass1 = null;
      ...
      try {
      	superClass =(CReferenceType) superClass.findType(self);
    ...
    if (superClass != null) {
      ...
      check(context, 
	    ...
	    KjcMessages.CLASS_ACCESSPARENT, superClass.getJavaName());
      check(context,
	    !clazz.isFinal(),
	    KjcMessages.CLASS_PARENT_FINAL, superClass.getJavaName());
      check(context,
	    !clazz.isInterface(),
	    KjcMessages.CLASS_EXTENDS_INTERFACE, superClass.getJavaName());
    }
    ...
    throwStatement
      =  new JThrowStatement(ref,
			     new JUnqualifiedInstanceCreation(ref,
							      context.getTypeFactory().createType("java/lang/NoClassDefFoundError",
												  false),
							      new JExpression[] {
