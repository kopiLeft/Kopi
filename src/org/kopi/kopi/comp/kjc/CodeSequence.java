...
public final class CodeSequence extends org.kopi.util.base.Utils implements Constants {
  ...
  private CodeSequence() {
    instructions = new Instruction[org.kopi.bytecode.classfile.Constants.MAX_CODE_PER_METHOD];
  ...
  public static CodeSequence getCodeSequence() {
    CodeSequence	seq;
    if (!org.kopi.bytecode.classfile.Constants.ENV_USE_CACHE || stack.empty()) {
   ...
   */
  public void release() {
    if (org.kopi.bytecode.classfile.Constants.ENV_USE_CACHE) {
      stack.push(this);
      handlers.clear();
      lines.clear();
      /* LOCAL VARIABLES NOT USED
  ...
  public final LocalVariableInfo[] getLocalVariableInfos() {
    ...
    for (Enumeration stacks = locals.elements(); stacks.hasMoreElements(); ) {
  ...
  private void resolveLabels() {
    ...
    if (labelAtEnd) {
      plantNoArgInstruction(org.kopi.bytecode.classfile.Constants.opc_nop);
    }
