...
public class VCMultiDimension extends VCDimension {
   ...
   */
  public VCMultiDimension(TokenReference where,
                          String ident,
                          int number,
                          int count,
                          String label,
  ...
  public static void generateFields(VCParseChartContext context,
				    ...
				    VKCommand[] commands,
				    VKTrigger[] triggers) {
    VCMultiDimension 		field;
    
    for (int i = 0; i < count; i++) {
      String	lab = label == null ?
	null :
	i < label.size() ? (String)label.get(i) : null; // WARNING !!!
      field = new VCMultiDimension(where, ident + (i + 1), i, count, lab, help, type, commands, triggers);
  ...
  public JFieldDeclaration genCode(CParseClassContext context) {
    if (getPosInArray() == 0) {
      ...
      context.addFieldDeclaration(VKUtils.buildFieldDeclaration(getTokenReference(),
								ACC_PUBLIC,
								new CArrayType(getType().getDef().getDimensionChartType(), 1),
								ident,
								null));
    }
