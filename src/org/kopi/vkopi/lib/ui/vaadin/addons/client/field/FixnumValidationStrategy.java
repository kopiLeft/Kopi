 ...
 */
public class FixnumValidationStrategy extends AllowAllValidationStrategy {
  
  //---------------------------------------------------
  // CONSTRUCTOR
  //---------------------------------------------------
  
  public FixnumValidationStrategy(int maxScale,
                                  boolean fraction,
                                  int width,
                                  Double minval,
                                  Double maxval)
  ...
  public void checkType(VInputTextField field, String text) throws CheckTypeException {
    if ("".equals(text)) {
      ...
      if (v!= null) {
        ...
        if (v.scale() > maxScale) {
          throw new CheckTypeException(field, "00011", maxScale);
        }
        
        if (minval != null && v.compareTo(new BigDecimal(minval)) == -1) {
          throw new CheckTypeException(field, "00012", minval);
        }
        if (maxval != null && v.compareTo(new BigDecimal(maxval)) == 1) {
      ...
      field.setText(toText(v.setScale(maxScale)));
    }
  }
   ...
   */
  private static BigDecimal scanBigDecimal(String str) {
