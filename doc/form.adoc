...
In this example, Objects is a type you should have defined previouslyin type definition section of the form using standard types, CODE types, ENUM types, LIST and SELECT command ...

==== Field Alignment

This command is used to define the localization of the field on the form. There are three types of alignment.

* ALIGN RIGHT in order to shift the field into the right side of the form,
* ALIGN LEFT to move it to the left side,
* ALIGN CENTER to place it in the middle. 
...
END FIELD
----
In this example, you can drag image files and drop them in the fiel named *  images".


==== Field Options
 
In this part of the field definintion, you can use one or more option from the 10 options available for fields in Kopi, here is the a list of these field options.
 ...
 * NO DETAIL            : If the block is in detailed, using this option on a field make it invesible in the detail.
 * NO CHART             : If the block is multiple, using this option on a field exclude it from the chart. 
 * QUERY UPPER          : Whatever the string you inuput, this option will make kopi transform it to capital letters.
 * QUERY LOWER          : the oposite of the previous option it transform strings to lowercase.
